...
def plot_residual_map(original, restored, damaged, save_path):
    ...
    fig.suptitle(
        "Neural Restoration Efficiency & Residual Error Map Analysis",
        fontsize=13,
        fontweight="bold",
        y=1.01,
    )
    ...
    axes[3].set_title(
        "Absolute Error Map (|GT - Out|)", fontsize=9, fontweight="semibold"
    )
...
def restore():
    try:
        ...
        if img_file and img_file.filename != "":
            ...
            clean_pil = (
                Image.open(filepath).convert("RGB").resize((256, 256), Image.LANCZOS)
            )
            # Save resized clean
            ...
            damaged_path = os.path.join(
                app.config["UPLOAD_FOLDER"], f"{uid}_damaged.png"
            )
            ...
            if not clean_path_orig.exists():
                return (
                    jsonify(
                        {"error": "Demo outputs not generated. Run dry-run first."}
                    ),
                    400,
                )
        ...
        paper_fig_path = os.path.join(
            app.config["UPLOAD_FOLDER"], f"{uid}_paper_figure.png"
        )
        plot_results(
            original=clean_pil,
            damaged=damaged_pil,
        ...
        residual_fig_path = os.path.join(
            app.config["UPLOAD_FOLDER"], f"{uid}_residual_figure.png"
        )
        plot_residual_map(
            original=clean_pil,
            restored=restored_pil,
