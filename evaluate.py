...
@torch.no_grad()
def run_lama(
    gen: Generator, damaged: torch.Tensor, mask: torch.Tensor, device
) -> torch.Tensor:
...
def get_args():
    ...
    p.add_argument(
        "--batch_size", type=int, default=1, help="Keep at 1 for easy visualisation"
    )
    p.add_argument("--num_workers", type=int, default=2)
    p.add_argument(
        "--n_vis", type=int, default=5, help="Number of results to visualise"
    )
    p.add_argument(
        "--no_lpips", action="store_true", help="Skip slow LPIPS computation"
    )
    return p.parse_args()
...
def main():
    ...
    for idx, batch in enumerate(test_loader):
        ...
        for tag, restored in [
            ...
            if use_lpips:
                results[tag]["lpips"].append(
                    compute_lpips(clean, restored, device=device)
                )
