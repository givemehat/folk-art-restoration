...
def run_tests():
    ...
    try:
        ...
        print(
            f"  ✓ Generator (With Attention Gates) forward pass: {dummy_out_attn.shape}"
        )
    ...
    try:
        ...
        recon_base = ReconstructionLoss(
            l1_weight=1.0, perceptual_weight=0.1, style_weight=0.0
        ).to(device)
        ...
        recon_style = ReconstructionLoss(
            l1_weight=1.0, perceptual_weight=0.1, style_weight=10.0
        ).to(device)
        loss_style, breakdown_style = recon_style(pred, target)
        print(
            f"  ✓ Reconstruction Loss + Style Consistency Loss: {loss_style.item():.4f}"
        )
        ...
        loss_style.backward()
        print(
            "  ✓ Gradient backpropagation through perceptual & style blocks succeeded!"
        )
    except Exception as e:
