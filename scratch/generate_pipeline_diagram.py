...
def main():
    ...
    # Define styles
    box_props_input = dict(boxstyle="round,pad=0.4", facecolor='#F1F5F9', edgecolor='#64748B', linewidth=1.5)
    box_props_stage1 = dict(boxstyle="round,pad=0.5", facecolor='#EFF6FF', edgecolor='#2563EB', linewidth=2.0)
    box_props_stage2 = dict(boxstyle="round,pad=0.5", facecolor='#FDF2F8', edgecolor='#DB2777', linewidth=2.0)
    box_props_data = dict(boxstyle="round,pad=0.4", facecolor='#ECFDF5', edgecolor='#10B981', linewidth=1.5)
    
    # 1. Damaged Input Box (Left-Center)
    ax.text(1.2, 3.0, "Damaged Input Image\n(256 $\\times$ 256 $\\times$ 3)", 
            ha="center", va="center", bbox=box_props_input, fontsize=9, fontweight='bold', color='#1E293B')
    
    # 2. Bicubic Downsample Box (Upper-Left)
    ax.text(3.8, 4.5, "Bicubic Downsample\n(128 $\\times$ 128 $\\times$ 3)", 
            ha="center", va="center", bbox=box_props_data, fontsize=8, color='#047857')
    
    # 3. Stage 1: EDSR Box
    ax.text(6.8, 4.5, "Stage 1: EDSR Network\nSuper-Resolution upscaling", 
            ha="center", va="center", bbox=box_props_stage1, fontsize=9, fontweight='bold', color='#1D4ED8')
    
    # 4. Auto-Masking Box (Lower-Left)
    ax.text(3.8, 1.5, "Heuristic Auto-Masking\n(White/Saturation Detect)", 
            ha="center", va="center", bbox=box_props_data, fontsize=8, color='#047857')
    
    # 5. Damage Mask Box
    ax.text(6.8, 1.5, "Binary Damage Mask\n(256 $\\times$ 256 $\\times$ 1)", 
            ha="center", va="center", bbox=box_props_input, fontsize=9, color='#1E293B')
    
    # 6. Feature Concatenation Circle/Box
    ax.text(8.8, 3.0, "Concat\n(4 channels)", 
            ha="center", va="center", 
            bbox=dict(boxstyle="circle,pad=0.3", facecolor='#F5F5F4', edgecolor='#78716C', linewidth=1.5), 
            fontsize=8, fontweight='semibold')
    
    # 7. Stage 2: LaMa Generator
    ax.text(8.8, 4.8, "Stage 2: LaMa\nInpainting Generator\n(FFC Bottleneck)", 
            ha="center", va="center", bbox=box_props_stage2, fontsize=9, fontweight='bold', color='#BE185D')
    
    # 8. Restored Output Box
    ax.text(5.0, 3.0, "Restored Output\n(256 $\\times$ 256 $\\times$ 3)", 
            ha="center", va="center", bbox=box_props_data, fontsize=9, fontweight='bold', color='#065F46')
