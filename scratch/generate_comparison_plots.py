...
def main():
    ...
    ax1.grid(
        True, which="both", linestyle="--", linewidth=0.5, color="#E2E8F0", zorder=0
    )
    ...
    rects1 = ax1.bar(
        ...
        edgecolor="#3730A3",
        linewidth=1.2,
        error_kw=dict(ecolor="#1E1B4B", lw=1.5, capsize=4, capthick=1.5),
        zorder=3,
    )

    ax1.set_ylabel(
        "PSNR Score (dB) [Higher is Better]",
        color="#4F46E5",
        fontweight="bold",
        fontsize=10,
    )
    ...
    rects2 = ax2.bar(
        ...
        edgecolor="#9D174D",
        linewidth=1.2,
        error_kw=dict(ecolor="#500724", lw=1.5, capsize=4, capthick=1.5),
        zorder=3,
    )

    ax2.set_ylabel(
        "SSIM Score [Higher is Better]", color="#EC4899", fontweight="bold", fontsize=10
    )
    ...
    # Title and labels
    plt.title(
        "Quantitative Reconstruction Analysis (PSNR vs SSIM)",
        fontsize=12,
        fontweight="bold",
        pad=15,
    )
    ...
    rects3 = ax.bar(
        ...
        linewidth=1.2,
        error_kw=dict(ecolor="#020617", lw=1.5, capsize=5, capthick=1.5),
        zorder=3,
    )
    ...
    print("\n" + "=" * 80)
    print(
        f"{'Model / Method':<30} | {'PSNR ↑ (dB)':<15} | {'SSIM ↑':<12} | {'LPIPS ↓':<12}"
    )
    print("=" * 80)
